...
interface Path { icon: string; to: string; label: string, iconHTML?: JSX.Element };
const Paths: { [key: string]: Path[] } = {
	"": [
		...
		},
		{
			icon: "support",
			to: "/download/spotify",
			iconHTML: (<svg className='spotify-svg' xmlns="http://www.w3.org/2000/svg" height="168px" width="168px" version="1.1" viewBox="0 0 168 168">
	...
	render() {
		return (
			<Link onClick={() => OnLinkClick.dispatch(0)} className="nav-item" to={this.props.Data.to} data-active={this.props.Path === this.props.Data.to || undefined}>
				{!this.props.Data.iconHTML ? <span className="material-icons">
					{this.props.Data.icon}
				</span> : this.props.Data.iconHTML}
